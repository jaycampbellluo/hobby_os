...
chap5
    ...
    we have asm functions which is how we access syscalls

    for a write syscall, this would look like:
    ...
    SYS_WRITE_NUMBER is the index into a sys call table
    w8 is a special register for storing sys call values and is read after we call svc
    svc is the way of generating an exception from a user space process

    after issuing the exception, we get some handler to handle it.

    exception handler:
        check the value in the esr_el1 register. this contains the type of exception class at some offset (we use ESR_ELx_EC_SHIFT) to get the value at the offset.
        if this value is equal to ESR_ELx_EC_SVC64, then the exception was generated by a call to svc and we branch to an el0_scv handler, else we return an error/handle an invalid entry

    el0_svc:
        we define a couple of aliases for different registers:
            sc_nr: x25      total syscalls
            scno: x26       syscall number
            stbl: x27       syscall table

        we load the syscall table pointer from sys_call_table into stbl
